...
Within \ac{LEX}, as with re-\ac{ECN}, such packets are labelled \ac{FNE}, with the \ac{IP} header being marked accordingly.
...
Neither of these approaches are strictly necessary in a mutualistic architecture. 
...
In such a case, a network only needs to perform path selection according to local policy and pass the information onto the end-host.
