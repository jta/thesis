...
Table \ref{table:limits} displays the extent to which each of these limitations affects inbound traffic in the MAWI dataset over time. 
...
As the weight of traditional carriers such as Cogent and \ac{NTT} has waned, \acp{AS} known to harbour one-click hosting services such as Choopa, Webazilla, WZ Communications, Carpathia and LeaseWeb have gained significance.
...
% explore the results
A critical component in determining the upperbound for the congestion window size is the negotiation of the \ac{TCP} \emph{windowscale} option during the initial handshake. 
...
Given the popularity of emulating a constant bit rate service over \ac{TCP}, that no such abstraction is provided at the socket level API is unfortunate.
