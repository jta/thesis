...
Having processed each daily trace individually, flow  proceed by aggregating results longitudinally in order to trace the evolution of constraints affecting \ac{TCP} across both time and spatial/topological dimensions. 
The ensuing analysis revisits four commonly held assumptions regarding Internet throughput. 
The aim in so doing is to provide a much needed factual verification of these assumptions, which itself can lead to a re-appraisal of Internet throughput modelling efforts, particularly those enunciated in Zhang et al.\ \cite{Zhang:2002p85}.
...
Given that throughput conditions on the receiver side improve as \acs{OS} upgrades are rolled out, and that the user base within Japan covered by \acs{MAWI} has remained relatively stable over time, all things being equal the whole region is expected to exhibit improvements as time progresses. 
Hence it stands to reason that any significant degradation in host limitations most probably does not stem from \acs{OS} rollback or large sets of users with outdated \acp{OS} joining the Japanese networks. 
Instead, a more likely explanation lies in macroscopic shifts in routing or application popularity which lead to a change in \emph{where} traffic originates from.
