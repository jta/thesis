...
Because of the assumption that the traffic is assigned in inverse proportion to loss rate then a branch with no loss would naturally have all the traffic assigned to it. 
To correct this, $R_i$ is incremented for each path by one maximum segment size (MSS). 
...
\begin{figure}
    ...
    {Simulation using PREFLEX to balance traffic over two paths. \label{fig:two}
...
Because only local linearity is assumed then large adjustments of the traffic split are likely to cause problems.  
...
Keeping the coefficient of variation within some bound $\delta$ is a measure of the amount by which an estimate is likely to vary from the true mean \footnote{note, however, that this is not technically a confidence interval.}.

For the number of lost packets on a given prefix $i$ the estimated \ac{CV} is $\hat{c_v(i)} = \sqrt{t_i \hat{p_i}(1- \hat{p_i})}/t_i\hat{p_i}$.
...
This gives an estimated minimum time period to set for the next time period.  In order to get weighted \ac{CV} of packet loss (and hence loss rate) equal to or below $\delta$ the time period $\tau'$ is bounded by
$$
