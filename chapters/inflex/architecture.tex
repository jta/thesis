...
The core of the architecture relies on repurposing the \ac{DS} field in each IP packet to provide an in-band signalling channel between the end-host and the inflector.
...
An Interior Forwarding $label$, which determines the plane over which a packet is forwarded, and an $echo$ bit, which explicitly signals a request from the host or a reply from the network.
...
\begin{figure}
    ...
    \end{subfigure}
    \caption{INFLEX architecture (above) and header (below). The edge switch forwards traffic across virtual planes set up by a centralized routing service.
...
These changes are all applied at the IP layer -- transport protocols need only to decide when to send inflection requests -- while applications can remain unchanged.
...
Firstly, INFLEX is only applicable to IP packets.
...
Any inbound IP traffic may potentially be INFLEX capable and as such can proceed to the next stage.
For outbound IP traffic, only packets with the INFLEX flag set require further processing.
